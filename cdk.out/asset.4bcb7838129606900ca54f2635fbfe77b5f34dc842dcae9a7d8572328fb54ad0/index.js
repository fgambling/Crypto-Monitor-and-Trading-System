...
var MARKET_CAP_MIN = Number(process.env.MARKET_CAP_MIN ?? 0);
var VOLUME_MIN = Number(process.env.VOLUME_MIN ?? 0);
...
var handler = async (event) => {
  ...
  for (const record of records) {
    ...
    for (const keyword of keywords) {
      ...
      const token = await findTokenOnBSC(keyword.trim());
      if (!token) {
        continue;
      }
...
async function findTokenOnBSC(keyword) {
  try {
    const response = await axios_default.get("https://api.dexscreener.com/latest/dex/search", {
      params: { q: keyword },
      timeout: 5e3
    });
    ...
    if (Number.isFinite(MARKET_CAP_MIN) && marketCapRaw < MARKET_CAP_MIN) {
      return void 0;
    }
    if (Number.isFinite(VOLUME_MIN) && volume24hRaw < VOLUME_MIN) {
      return void 0;
    }
    return {
      ...
      marketCapUsd: Number.isFinite(marketCapRaw) ? marketCapRaw : void 0,
      volume24hUsd: Number.isFinite(volume24hRaw) ? volume24hRaw : void 0
    };
  } catch (err) {
    console.warn("DexScreener search failed for keyword", keyword, err);
    return void 0;
  }
}
