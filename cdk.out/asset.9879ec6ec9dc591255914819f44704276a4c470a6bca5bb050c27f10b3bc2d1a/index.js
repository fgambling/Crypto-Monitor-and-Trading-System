...
var SYMBOL_CACHE = /* @__PURE__ */ new Map();
...
async function ensureSymbol(symbol) {
  ...
  try {
    const response = await axios_default.get("https://api.binance.com/api/v3/exchangeInfo", {
      params: { symbol },
      timeout: 5e3
    });
    ...
    if (axios_default.isAxiosError(err) && err.response?.status === 400) {
...
async function fetchPrice(symbol) {
  try {
    const response = await axios_default.get("https://api.binance.com/api/v3/ticker/price", {
      params: { symbol },
      timeout: 5e3
    });
